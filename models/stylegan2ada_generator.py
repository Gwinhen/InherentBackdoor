...
class StyleGAN2ADAGenerator(nn.Module):
    ...
    def forward(self,
        ...
        synthesis_results = self.synthesis(wp, randomize_noise, special_styles=special_styles, special_noises=special_noises, return_featuremap=return_featuremap)
...
class MappingModule(nn.Module):
    ...
    def __init__(self,
        ...
        if label_size:
            self.add_module('LabelEmbed',
                            DenseBlock(in_channels=label_size,
                                       out_channels=input_space_dim,
                                       use_wscale=self.use_wscale,
...
class SynthesisModule(nn.Module):
    ...
    def __init__(self,
        ...
        for res_log2 in range(self.init_res_log2, self.final_res_log2 + 1):
            ...
            if res_log2 == self.final_res_log2 or self.architecture == 'skip':
                ...
                self.add_module(
                    layer_name, ModulateConvBlock(in_channels=self.get_nf(res),
    ...
    def forward(self, wp, randomize_noise=False, special_styles=None, special_noises=None, return_featuremap=False):
        results = {'wp': wp}
        x = self.early_layer(wp[:, 0])
        if return_featuremap:
            results['featuremap00'] = x.detach().clone()
        ...
        if self.architecture == 'origin':
            ...
            if special_styles is None or len(special_styles) == 0:
                special_styles = [None] * (self.num_layers - 1)
            if special_noises is None or len(special_noises) == 0:
                special_noises = [None] * ((self.final_res_log2 + 1 - self.init_res_log2) * 2)
            for layer_idx in range(self.num_layers - 1):
                x, style = self.__getattr__(f'layer{layer_idx}')(
                    x, wp[:, layer_idx], randomize_noise, special_styles[layer_idx], special_noises[layer_idx])
                results[f'style{layer_idx:02d}'] = style
                if return_featuremap:
                    results[f'featuremap{layer_idx+1:02d}'] = x.detach().clone()
                if layer_idx % 2 == 0:
...
class ModulateConvBlock(nn.Module):
    ...
    def forward(self, x, w, randomize_noise=False, special_style=None, special_noise=None):
        batch = x.shape[0]
