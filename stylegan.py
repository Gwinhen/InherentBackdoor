import math
import torch
from models.stylegan2ada_generator import StyleGAN2ADAGenerator
...
class Fake_G:
    ...
    def __call__(self, code, randomize_noises=None,
                 special_noises=None, special_styles=None):
        return self.g_function(
                    code,
                    randomize_noises=randomize_noises,
...
def build_generator(device, checkpoint_path):
    ...
    def fake_generator(code, randomize_noises=None,
                       special_noises=None, special_styles=None):
        # Sample and synthesize
        all_results = generator(
                            code,
                            **synthesis_kwargs,
                            use_w_space=use_w_space,
                            randomize_noises=randomize_noises,
                            special_noises=special_noises,
                            special_styles=special_styles
                      )
...
class StyleGAN:
    def __init__(self, device):
        ...
        # Use genforce model
        self.num_layers = 8
        genforce_model = './ckpt/stylegan2ada.pth'
        self.generator = build_generator(self.device, genforce_model)
    
    def get_w_mean(self, batch_labels):
        with torch.no_grad():
            latent_z_inputs = torch.randn(batch_labels.size(0),
                                          self.latent_dim,
                                          device=self.device)
            latent_w = self.generator.G.mapping(latent_z_inputs,
